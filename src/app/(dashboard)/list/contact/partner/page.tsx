...
import TableSearchContactP from '@/components/TableSearchContactP';
...
const renderRow = (item: ContactList) => (
  <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurple1Light">
    ...
    <td className="hidden md:table-cell">{item.partner?.name}</td>
  ...
  {item.partner?.country ? (
    <>
      <Image
        src={getFlagImageUrl(item.partner.country)}
        alt={item.partner.country}
        width={24}
        height={16}
        className="rounded-sm border border-gray-300"
      />
      <span>{item.partner.country}</span>
    </>
  ) : (
    <span>N/A</span>
  )}
</td>
    <td className="hidden md:table-cell">{item.partner?.city}</td>
    <td>
...
const ContactListPartnerPage = async({
  searchParams,
}: {
  ...
  return (
    <div className='bg-white p-4 rounded-md flex-1 m-4 mt-0 h-full'>
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Partner's contacts</h1>
...
export default ContactListPartnerPage