...
import { Ad, Player, Prisma } from '@prisma/client';
import Image from 'next/image'
import Link from 'next/link';


type AnnouncementList = Ad & {author:Player} 
...
const renderRow = (item: AnnouncementList) => (
  <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurple1Light">
    ...
    <td className="hidden md:table-cell">{item.author.name ?? "Unknown"}
      <h3 className="">{item.author.surname ?? "Unknown"}</h3>
    </td>
    ...
    <td>
      <div className="flex items-center gap-2">
        ...
        {(role === "admin" || role === "comercial") && (
          <>
             <FormModal table="ad" type="update" id={item.id} />
             <FormModal table="ad" type="delete" id={item.id} />
          </>
        )}
      </div>
    </td>
...
const AnnouncementListPage = async({
  ...
  const query: Prisma.AdWhereInput = {};
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
           ...
           <div className="flex items-center gap-4 self-end">
              ...
              {role === "admin" && (
              <FormModal table="ad" type="create"/>
            )}
           </div>
        </div>
      </div>
