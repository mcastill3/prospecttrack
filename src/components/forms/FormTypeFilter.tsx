...
const FormTypeFilter = () => {
  const [type, setType] = useState<string>(""); // Enterprise or SMB
  ...
  useEffect(() => {
    if (!type || !showTable) return;

    const fetchCompanies = async () => {
      const res = await fetch(
        `/api/export/list/type?type=${type}&page=${page}&pageSize=${ITEM_PER_PAGE}`
      );
  ...
  const handleDownload = async () => {
    ...
    try {
      const resAll = await fetch(
        `/api/export/list/type?type=${type}&page=1&pageSize=100000`
      );
      ...
      const excelRes = await fetch("/api/export/list/excel/type", {
        method: "POST",
      ...
      a.href = url;
      a.download = `prospection_list_by_type_${type}.xlsx`;
      a.click();
      a.remove();
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex flex-col space-y-2">
          <h1 className="text-3xl font-bold">List Generator - Company Type</h1>
          <p className="text-gray-500">Filter companies by type and download the list</p>
        </div>
        ...
        <div>
          ...
          <Select value={type} onValueChange={setType}>
            ...
            <SelectContent className="max-h-60 overflow-auto rounded-md border bg-white shadow-lg">
              <SelectItem value="Enterprise" className="px-4 py-2 text-gray-900">
                Enterprise
              </SelectItem>
              <SelectItem value="SMB" className="px-4 py-2 text-gray-900">
                SMB
              </SelectItem>
            </SelectContent>
          </Select>
