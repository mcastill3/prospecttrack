...
import TableCountryGenerator from "@/components/Tables/TableCountryGenerator"; // Asegúrate de crearlo
...
const FormCountryFilter = () => {
  ...
  useEffect(() => {
    ...
    const fetchCountries = async () => {
      const res = await fetch("/api/generator/companies/countries");
  ...
  const router = useRouter(); // agrégalo arriba
...
const handleSubmit = (e: React.FormEvent) => {
  ...
  // Fetch empresas cada vez que cambie país o página
 useEffect(() => {
  if (!countryId || !showTable) return; // Solo hace fetch si showTable es true

  const fetchCompanies = async () => {
    const res = await fetch(
      `/api/export/list/country?countryId=${countryId}&page=${page}&pageSize=${ITEM_PER_PAGE}`
    );
    const data = await res.json();

    setContactsData(data.items);
    setRecordCount(data.total);
    // No pongas setShowTable(false) acá, que eso oculta la tabla
  };

  fetchCompanies();
}, [countryId, page, showTable]);
...
const fetchAllContacts = async (countryId: string): Promise<Contact[]> => {
  // Aquí asumimos que tu API acepta pageSize alto para traer todo
  const res = await fetch(
    `/api/export/list/country?countryId=${countryId}&page=1&pageSize=10000`
  );
  const data = await res.json();
  return data.items;
};
  ...
  const handleDownload = async () => {
    ...
    try {
      const resAll = await fetch(
        `/api/export/list/country?countryId=${countryId}&page=1&pageSize=100000`
      );
      ...
      const excelRes = await fetch("/api/export/list/excel/country", {
        method: "POST",
        headers: {
      ...
      a.href = url;
      a.download = "prospection_list_by_country.xlsx";
      a.click();
      a.remove();
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex flex-col space-y-2">
          <h1 className="text-3xl font-bold">List Generator - Country</h1>
          <p className="text-gray-500">Filter companies by country and download the list</p>
        </div>
      ...
      >
        <div>
          <label htmlFor="country-select" className="block mb-2 text-sm font-semibold text-gray-700">
            Select Country
          </label>
        ...
        {showTable && (
          ...
          >
            ...
            <div className="bg-white p-4 rounded-b-lg">
              <TableCountryGenerator contacts={contactsData} />
...
export default FormCountryFilter;