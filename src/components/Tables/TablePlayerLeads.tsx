...
const TablePlayerLeads = async ({ playerId }: { playerId: string }) => {
  // Obtener los leads asociados al jugador
  const leads = await prisma.lead.findMany({
    where: {
      players: {
        some: { id: playerId }, // Filtrar por el player
      },
    },
    include: {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg">
      <h1 className="text-2xl font-semibold text-gray-800">Player Leads</h1>
...
export default TablePlayerLeads;
