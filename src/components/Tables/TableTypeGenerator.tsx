...
interface Contact {
  ...
  type?: "Enterprise" | "SMB"; // Agregamos el tipo opcional
}
...
const TableTypeGenerator = ({ contacts }: Props) => {
  if (!contacts.length) {
    ...
    <Table className="min-w-full border-collapse">
      <TableHeader className="bg-gray-900 text-white font-semibold">
        <TableRow>
          ...
          <TableHead className="py-4 px-6 text-left text-white">Type</TableHead> {/* Nueva columna */}
      ...
      <TableBody className="bg-white">
        {contacts.map((contact) => (
          <TableRow key={contact.email} className="border-b">
...
export default TableTypeGenerator;
